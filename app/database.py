...
def init_db():
    ...
    try:
        ...
        logger.error(f"Error initializing database at {DB_PATH}: {str(e)}", exc_info=True)
        raise
...
def create_lot(lot_data: dict, farmer_hash: str):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"Created lot {lot_data['lot_id']} successfully")
    except Exception as e:
        logger.error(f"Error creating lot {lot_data.get('lot_id', 'unknown')}: {str(e)}", exc_info=True)
        raise
...
def update_transporter(lot_id: str, transport_data: dict, transporter_hash: str):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"Updated transporter info for lot {lot_id}")
    except Exception as e:
        logger.error(f"Error updating transporter for lot {lot_id}: {str(e)}", exc_info=True)
        raise
...
def update_distributor(lot_id: str, distributor_data: dict, distributor_hash: str):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"Updated distributor info for lot {lot_id}")
    except Exception as e:
        logger.error(f"Error updating distributor for lot {lot_id}: {str(e)}", exc_info=True)
        raise
